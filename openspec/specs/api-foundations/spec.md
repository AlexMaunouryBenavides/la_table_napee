# api-foundations Specification

## Purpose

Les comportements transverses de l'API — appliqués à toutes les requêtes, posés une
fois au bootstrap et au module racine : validation des entrées, CORS crédentialisé,
forme d'erreur cohérente, sérialisation de sortie sans fuite, durcissement
(helmet + throttler), et configuration validée au démarrage. La plateforme sur
laquelle s'appuient toutes les features, avec sécurité et cohérence par défaut.
